...
class LowLevelRefactoring:
    ...
    def __init__(self, name, level, commit_threshold):
        self._name = name
        self._level = level
    ...
    def get_non_refactored_instances(self, dataset: str = ""):
        ...
        """
        return execute_query(get_level_stable(int(self._level), self._commit_threshold, dataset))
    ...
    for level in selected_level:
        for refactoring in LEVEL_MAP[level]:
            for commit_threshold in LEVEL_Stable_Thresholds_MAP[level]:
                all_refactorings += [LowLevelRefactoring(refactoring, level, commit_threshold)]
