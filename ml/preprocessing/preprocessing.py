...
from configs import SCALE_DATASET, BALANCE_DATASET, DROP_METRICS, \
    DROP_PROCESS_AND_AUTHORSHIP_METRICS, PROCESS_AND_AUTHORSHIP_METRICS, DROP_FAULTY_PROCESS_AND_AUTHORSHIP_METRICS, SEED, TRAINING_SAMPLE_RATIO
...
def retrieve_labelled_instances(
    ...
    """
    log(
        f"---- Retrieve labeled instances for dataset: {datasets} and the refactoring {refactoring.name()}")
    ...
    log(
        f"raw number of refactoring instances: {refactored_instances.shape[0]}")
    log(
        f"raw number of non-refactoring with K={refactoring.commit_threshold()} instances: {non_refactored_instances.shape[0]}")
    ...
    if refactored_instances.shape[0] == 0:
        log(
            f"No refactorings found for refactoring type: {refactoring.name()}")
        return None, None, None

    if non_refactored_instances.shape[0] == 0:
        log(
            f"No non-refactorings found for threshold: {refactoring.commit_threshold()}")
        return None, None, None
    # test if any refactorings were found for the given refactoring type
    ...
    if is_training_data and 0 < TRAINING_SAMPLE_RATIO < 1 and not BALANCE_DATASET:
        refactored_instances, non_refactored_instances = sample_reduction(
            refactored_instances, non_refactored_instances, TRAINING_SAMPLE_RATIO)
    ...
    if DROP_FAULTY_PROCESS_AND_AUTHORSHIP_METRICS and not DROP_PROCESS_AND_AUTHORSHIP_METRICS:
        ...
        metrics = [
            metric for metric in PROCESS_AND_AUTHORSHIP_METRICS if metric in merged_dataset.columns.values]
    ...
    log(f"Got {x.shape[0]} instances with {x.shape[1]} features for the dataset: {datasets} at threshold {refactoring.commit_threshold()}.")
    return x, y, scaler
