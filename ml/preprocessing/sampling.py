...
def sample_reduction(positive_samples, negative_samples, ratio: float):
    ...
    if fraction_positive > 1.0:
        fraction_negative = len(positive_samples.index) * ((1 - ratio) / ratio) / len(negative_samples.index)
        fraction_positive = 1.0
    # negative limits
    elif fraction_negative > 1.0:
        fraction_positive = len(negative_samples.index) * (ratio / (1 - ratio)) / len(positive_samples.index)
    ...
    log(f"Reduced the number of samples to {len(positive_samples.index)}/ {len(negative_samples.index)} ({ratio}/ {1-ratio})", False)
