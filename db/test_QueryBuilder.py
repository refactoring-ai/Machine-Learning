...
from db.QueryBuilder import join_table, get_metrics_level, get_instance_fields, \
...
class QueryBuilderUnitTest(unittest.TestCase):
    ...
    def test_get_instance_fields(self):
        sqlExpected: str = "SELECT RefactoringCommit.id, RefactoringCommit.className, CommitMetaData.commitId FROM RefactoringCommit INNER JOIN CommitMetaData ON RefactoringCommit.commitMetaData_id = CommitMetaData.id INNER JOIN project ON RefactoringCommit.project_id = project.id"
        sqlBuilt: str = get_instance_fields("RefactoringCommit",
                                            [("RefactoringCommit", ["id", "className"]), ("CommitMetaData", ["commitId"]), ("project", [])])
        ...
        sqlExpected += " AND datasetName = \"github\""
        sqlBuilt: str = get_instance_fields("RefactoringCommit",
                                            [("RefactoringCommit", ["id", "className"]), ("CommitMetaData", ["commitId"]), ("project", [])],
                                            "CommitMetaData.parentCommitId != null",
                                            "github")
